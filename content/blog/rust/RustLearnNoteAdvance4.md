...
## 声明宏
声明宏（declarative macros）允许编写一些类似`match`表达式的控制结构，将一个值和包含相关代码的**模式**进行比较，一旦匹配成功，每个模式的相关代码会替换传递给宏的代码。所有这一切都发生于编译时。rust 会通过`std::prelude`自动引入宏，无需手动声明。。
...
来看`!vec`的简化版本定义，这里之所以是简化版是因为省略了预分配空间的代码。
```rust,name=lib.rs
#[macro_export]
macro_rules! vec {
    ( $( $x:expr ),* ) => {
...
```
接下来，我们再来看看与模式相关联、在`=>`之后的代码。`$()`中的部分将根据模式匹配的次数生成对应的代码，也就是说有几个元素就执行几次，本例中就是循环执行了 3 次`temp_vec.push($x);`。当调用`vec![1, 2, 3]`时，下面这段生成的代码将替代传入的源代码：
```rust
{
    let mut temp_vec = Vec::new();
    temp_vec.push(1);
...
```
然后在主项目的`Cargo.toml`中使用`dependencies`或`workspace`引入过程宏的包。这里就先用 dependencies。
...
这是 deno flask test 的 0.1.0 版本，[源码](https://github.com/denoland/flaky_test/blob/v0.1.0/src/lib.rs)。这个宏内容比较简单，适合用于学习。请看下面的注释。
```rust
  ...
  TokenStream::from(quote! {
    ...
    fn #name() {
      ...
      for i in 0..3 {
        ...
        // catch_unwind 捕获闭包执行过程中的 panic
        // 它接收一个闭包并执行，如果闭包 panic，返回 Result::Err，否则返回Result::Ok
        let r = std::panic::catch_unwind(|| {
            #name();
        });
        // 返回Result::Ok 时直接 return
        if r.is_ok() {
            return;
        }
        // 否则重试，重试到第三次还不行再抛出异常
...
1. 为每个类型手动实现该特质，就像之前特质章节所做的
2. 使用过程宏来统一实现该特质，这样用户只需要对类型进行标记即可：`#[derive(HelloMacro)]`
