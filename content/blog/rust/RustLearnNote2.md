...
&str：字符串**字面量**，字符串的不可变**切片（Slice）**引用，栈上分配，UTF-8 编码，由指针和长度构成。

> 字面量其实就是 literal，与变量相对，表示直接写在代码中的固定值。

> 切片是对集合的部分引用，因此不仅仅字符串有切片，数组等其它集合类型也有。如一个`i32`数组的切片类型就是`&[i32]`。
...
上面的例子是普通结构体。Rust 还包括元组结构体（有结构体名字没有属性名字，看起来像一个元组）和单元结构体（没有属性，用于标记类型或实现 trait，但不携带数据）。如下：
```rust
struct Common(f64, i32); // 元组结构体，只关心顺序和类型不关心属性命名，通过 self.0 访问元素
struct Placeholder; // 单元结构体，看起来什么也没有
```
...
基础类型、元组和数组、结构体与枚举都会存储在栈上；Box、Rc、String、集合（Vec等）会指向堆。注意如果结构体、枚举中包含堆上的类型，这些属性也会指向堆（但结构体本身还是在栈上）。
...
[^1]: 在 Rust 中，所有权转移并不是一个危险的事情，不再使用的变量就应该被及时消耗，否则代码后续将很难维护。我们应正确看待所有权转移，无需感到恐惧。
